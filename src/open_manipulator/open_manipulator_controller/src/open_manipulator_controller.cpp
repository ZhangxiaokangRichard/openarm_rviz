  ...
  timer_thread_state_(false)
{
  ...
  ************************************************************/
  ...
  use_rviz_             = priv_node_handle_.param<bool>("use_rviz", false);
  ...
  ************************************************************/
  ...
  else if (use_rviz_)                 log::info("Ready to simulate " + priv_node_handle_.getNamespace() + " on RViz");
  else                                log::info("Ready to simulate " + priv_node_handle_.getNamespace() + " on Gazebo");
...
void OpenManipulatorController::initPublisher()
{
  ...
  if (using_platform_ == true || use_rviz_)
  {
...
{
  if (using_platform_ == true || use_rviz_)  publishJointStates();
